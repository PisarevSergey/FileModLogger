...
namespace
{
  NTSTATUS set_stream_context(PFLT_CALLBACK_DATA data)
  {
    ...
    if (NT_SUCCESS(stat))
    {
      im(CREATE_DISPATCH, "allocate_stream_context success");
      ...
      if (NT_SUCCESS(stat))
      {
        im(CREATE_DISPATCH, "FltSetStreamContext success");
        new_ctx->adjust_refcount_on_post_create();
      }
      else if ((STATUS_FLT_CONTEXT_ALREADY_DEFINED == stat) && old_ctx.operator bool())
      {
        im(CREATE_DISPATCH, "context already set, using old context");
        old_ctx->adjust_refcount_on_post_create();
        stat = STATUS_SUCCESS;
      }
      else
      {
        em(CREATE_DISPATCH, "FltSetStreamContext failed with status %!STATUS!", stat);
      }
    }
    else
    {
      em(CREATE_DISPATCH, "allocate_stream_context failed with status %!STATUS!", stat);
    }
  ...
  NTSTATUS set_stream_handle_context(PFLT_CALLBACK_DATA data)
  {
    ...
    if (NT_SUCCESS(stat))
    {
      im(CREATE_DISPATCH, "allocate_stream_handle_context success");
      ...
      if (NT_SUCCESS(stat))
      {
        im(CREATE_DISPATCH, "FltSetStreamHandleContext success");
      }
      else if ((STATUS_FLT_CONTEXT_ALREADY_DEFINED == stat) && old_ctx.operator bool())
      {
        im(CREATE_DISPATCH, "stream handle context already set");
        stat = STATUS_SUCCESS;
      }
      else
      {
        em(CREATE_DISPATCH, "FltSetStreamHandleContext failed with status %!STATUS!", stat);
      }
    }
    else
    {
      em(CREATE_DISPATCH, "allocate_stream_handle_context failed with status %!STATUS!", stat);
    }
                                                   ...
                                                   _In_     FLT_POST_OPERATION_FLAGS Flags)
{
  ...
  {
    im(CREATE_DISPATCH, "post create is not draining");

    if (NT_SUCCESS(Data->IoStatus.Status) && (STATUS_REPARSE != Data->IoStatus.Status))
    {
      im(CREATE_DISPATCH, "create success and not reparse");

      BOOLEAN is_dir;
      NTSTATUS stat(FltIsDirectory(Data->Iopb->TargetFileObject, Data->Iopb->TargetInstance, &is_dir));
      if (NT_SUCCESS(stat) && (FALSE == is_dir))
      {
        im(CREATE_DISPATCH, "file opened is not directory");
        if (FALSE == FsRtlIsPagingFile(Data->Iopb->TargetFileObject))
        {
          im(CREATE_DISPATCH, "file opened is not paging file");
          stat = set_stream_context(Data);
          if (NT_SUCCESS(stat))
          {
            im(CREATE_DISPATCH, "set_stream_context success");
            stat = set_stream_handle_context(Data);
            if (NT_SUCCESS(stat))
            {
              im(CREATE_DISPATCH, "set_stream_handle_context success");
            }
            else
            {
              em(CREATE_DISPATCH, "set_stream_handle_context failed with status %!STATUS!", stat);
            }
          }
          else
          {
            em(CREATE_DISPATCH, "set_stream_context failed with status %!STATUS!", stat);
          }
        }

        if (!NT_SUCCESS(stat) && (0 == (Data->Iopb->TargetFileObject->Flags & FO_HANDLE_CREATED)))
        {
          em(CREATE_DISPATCH, "post create dispatching failed with status %!STATUS!, canceling open", stat);
